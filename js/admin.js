...
async function loadAdminUsers() {
  ...
  tbody.innerHTML=snap.docs.map(doc=>{
    const u=doc.data();
    const dot=`width:14px;height:14px;border-radius:50%;background:${u.avatarColor||"#D4A017"};display:inline-block;vertical-align:middle;margin-right:6px`;
    return `<tr>
      <td><span style="${dot}"></span>${u.displayName||"-"}</td>
      ...
      <td><button class="btn-sm btn-edit" onclick="editUser('${doc.id}')">Editar</button></td>
    </tr>`;
  }).join("");
}
...
document.getElementById("btn-new-user").addEventListener("click",()=>{
  ...
  document.getElementById("modal-user-title").textContent="Agregar Guarisma";
...
async function editUser(uid) {
  ...
  const u=doc.data();
  document.getElementById("user-modal").classList.remove("hidden");
  document.getElementById("modal-user-title").textContent="Editar Guarisma";
...
function renderColorPicker(cid,selected,hid) {
  const container=document.getElementById(cid);
  if (!container) return;
  container.innerHTML=AVATAR_COLORS_ADMIN.map(c=>`
    <div onclick="selectColor('${c}','${hid}')"
      style="width:24px;height:24px;border-radius:50%;background:${c};cursor:pointer;border:2px solid ${c===selected?"#fff":"transparent"}"
      data-color="${c}"></div>`).join("");
  document.getElementById(hid).value=selected;
}

function selectColor(color,hid) {
  document.querySelectorAll("[data-color]").forEach(el=>{
    el.style.border=el.dataset.color===color?"2px solid #fff":"2px solid transparent";
  });
  document.getElementById(hid).value=color;
}
...
async function loadAdminMatches() {
  ...
  tbody.innerHTML=snap.docs.map(doc=>{
    ...
    const scoreStr=m.status==="finished"
      ?`${m.scoreHome}-${m.scoreAway}${m.penaltyWinner?" (pen:"+m.penaltyWinner+")":""}`
      :"-";
    const badge={scheduled:"\uD83D\uDD52 Programado",live:"\uD83D\uDFE2 En vivo",finished:"\u2705 Finalizado"};
    return `<tr>
      <td>${dtStr}</td><td>${m.home} vs ${m.away}</td><td>Grupo ${m.group}</td>
      <td>${badge[m.status]||m.status}</td><td>${scoreStr}</td>
      <td>
...
async function enterScore(matchId,home,away) {
  ...
  if (isKnockout&&hs===as) {
    const pen=prompt(`Empate ${hs}-${as}. \u00bfQui\u00e9n gan\u00f3 en penales?\n1: ${home}\n2: ${away}`);
  ...
  await db.collection("matches").doc(matchId).update({
    scoreHome:hs,scoreAway:as,penaltyWinner,status:"finished"
  });
...
function buildWeekOptions() {
  ...
  weeks.forEach(w=>{
    ...
    opt.value=JSON.stringify(w);opt.textContent=w.label;
...
async function renderWeek() {
  ...
  if (!weekIds.size){
    tbody.innerHTML=`<tr><td colspan="5" style="text-align:center;padding:2rem;color:#888">No hay partidos finalizados.</td></tr>`;
    document.getElementById("semanal-match-count").textContent="0 partidos";return;
  }
  document.getElementById("semanal-match-count").textContent=`${weekIds.size} partido(s)`;
  ...
  tbody.innerHTML=rows.map((r,i)=>{
    ...
    return `<tr>
      ...
      <td style="font-family:'Bebas Neue',sans-serif;font-size:1.3rem;color:${r.pts>0?"#D4A017":"#888"}">${r.pts}</td>
...
async function loadActividad() {
  ...
  const totalM=mSnap.size;
  const predCount={};
  pSnap.docs.forEach(d=>{const uid=d.data().userId;predCount[uid]=(predCount[uid]||0)+1;});
  const rows=uSnap.docs.map(d=>{
    const u=d.data();
    return {name:u.displayName||u.username,count:predCount[d.id]||0,points:u.totalPoints||0,color:u.avatarColor||"#D4A017"};
  ...
  function isLight(hex){try{const r=parseInt(hex.slice(1,3),16),g=parseInt(hex.slice(3,5),16),b=parseInt(hex.slice(5,7),16);return(r*299+g*587+b*114)/1000>128;}catch{return false;}}
  tbody.innerHTML=rows.map((r,i)=>{
    const pct=totalM>0?Math.round((r.count/totalM)*100):0;
    const barColor=r.count===0?"#222":pct>=50?"#D4A017":"#457b9d";
    const badge=r.count===0?`<span class="badge badge-off">Sin actividad</span>`:`<span class="badge badge-on">Activo</span>`;
    ...
    return `<tr>
      ...
      <td style="text-align:center"><strong style="color:${r.count===0?"#888":"#fff"}">${r.count}</strong><span style="color:#888;font-size:.75rem"> / ${totalM}</span></td>
      <td><div style="background:#1a1a1a;height:6px;border-radius:3px;overflow:hidden;width:100%;min-width:80px"><div style="background:${barColor};height:6px;width:${pct}%"></div></div><span style="font-size:.68rem;color:#888">${pct}%</span></td>
      <td>${badge}</td>
    </tr>`;
  }).join("");
}
